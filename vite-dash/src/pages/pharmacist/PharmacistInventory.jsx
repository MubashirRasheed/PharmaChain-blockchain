...
const PharmacistInventory = () => {
  ...
  async function getAllProducts() {
    try {
      ...
      setPharmacistInventoryData(response.data);
  ...
  const handleAddItem = () => {
    ...
    axios
      ...
      .then((response) => {
        const { data } = response;
        setPharmacistInventoryData([...pharmacistInventoryData, data]);
        setNewItem({
          name: '',
          productID: '',
          sku: '',
  ...
  const handleUpdateItem = () => {
    ...
    fetch(`${import.meta.env.VITE_BASE_URL}/pharmacyproducts/updatePharmacyProduct/${newItem.productID}`, {
      ...
      .then((response) => response.json())
      .then((data) => {
        const updatedPharmacistInventory = pharmacistInventoryData.map((item) => (item.id === data.id ? data : item));
        setPharmacistInventoryData(updatedPharmacistInventory);
        setNewItem({
          name: '',
          productID: '',
          sku: '',
  ...
  const handleDeleteItem = () => {
    ...
    fetch(`${import.meta.env.VITE_BASE_URL}/pharmacyproducts/detelePharmacyProduct/${newItem.productID}`, {
      ...
      .then((response) => {
        if (response.ok) {
          const updatedPharmacistInventory = pharmacistInventoryData.filter((item) => item.id !== newItem.productID);
          setPharmacistInventoryData(updatedPharmacistInventory);
          setNewItem({
            name: '',
            productID: '',
  ...
  console.log(pharmacistInventoryData);

  const rows = pharmacistInventoryData.map((user, index) => ({
    id: user.id,
    name: user.name,
    sku: user.sku,
    price: user.price,
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center">
        <h1 className="text-3xl font-bold mb-5">Pharmacist Inventory Management</h1>
      ...
      <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
        <Header title="Pharmacist Inventory Items" />
