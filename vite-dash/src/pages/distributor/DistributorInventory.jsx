...
const DistributorInventory = () => {
  // Latest Changings
  const apiUrl = `${import.meta.env.VITE_BASE_URL}/distributor/allDistributors`;
  const [distributorInventoryData, setDistributorInventoryData] = useState([]);
  ...
  async function getAllProducts() {
    try {
      ...
      setDistributorInventoryData(response.data);
  ...
  const handleAddItem = () => {
    // add new item to distributorInventory
    console.log('additem ', newItem);
    axios
      .post(`${import.meta.env.VITE_BASE_URL}/distributor/addNewDistributor`, newItem, {
        headers: {
      ...
      .then((response) => {
        const { data } = response;
        setDistributorInventoryData([...distributorInventoryData, data]);
        setNewItem({
          name: '',
          id: '',
          sku: '',
  ...
  const handleUpdateItem = () => {
    // update existing item in DistributorInventory
    console.log('update item', newItem);
    fetch(`${import.meta.env.VITE_BASE_URL}/distributor/updateDistributor/${newItem.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      .then((response) => response.json())
      .then((data) => {
        const updatedDistributorInventory = distributorInventoryData.map((item) => (item.id === data.id ? data : item));
        setDistributorInventoryData(updatedDistributorInventory);
        setNewItem({
          name: '',
          id: '',
          sku: '',
  ...
  const handleDeleteItem = () => {
    // delete item from DistributorInventory
    fetch(`${import.meta.env.VITE_BASE_URL}/distributor/deteleDistributor/${newItem.id}`, {
      method: 'DELETE',
    })
      .then((response) => {
        if (response.ok) {
          const updatedDistributorInventory = distributorInventoryData.filter((item) => item.id !== newItem.id);
          setDistributorInventoryData(updatedDistributorInventory);
          setNewItem({
            name: '',
            id: '',
  ...
  console.log(distributorInventoryData);

  const rows = distributorInventoryData.map((user, index) => ({
  ...
  return (
    <div>
      ...
      <div className="flex flex-col items-center">
        <h1 className="text-3xl font-bold mb-5">Distributor Inventory Management</h1>
      ...
      <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
        <Header title="Distributor Inventory Items" />
