...
const SupplierInventory = () => {
  // Latest Changings
  const apiUrl = `${import.meta.env.VITE_BASE_URL}/supplier/allSuppliers`;
  const [supplierInventoryData, setSupplierInventoryData] = useState([]);
  ...
  async function getAllProducts() {
    try {
      ...
      setSupplierInventoryData(response.data);
  ...
  const handleAddItem = () => {
    // add new item to SupplierInventory
    console.log('additem ', newItem);
    axios
      .post(`${import.meta.env.VITE_BASE_URL}/supplier/addNewSupplier`, newItem, {
        headers: {
      ...
      .then((response) => {
        const { data } = response;
        setSupplierInventoryData([...supplierInventoryData, data]);
        setNewItem({
          name: '',
          id: '',
          sku: '',
  ...
  const handleUpdateItem = () => {
    // update existing item in SupplierInventory
    console.log('update item', newItem);
    fetch(`${import.meta.env.VITE_BASE_URL}/supplier/updateSupplier/${newItem.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      .then((response) => response.json())
      .then((data) => {
        const updatedSupplierInventory = supplierInventoryData.map((item) => (item.id === data.id ? data : item));
        setSupplierInventoryData(updatedSupplierInventory);
        setNewItem({
          name: '',
          id: '',
          sku: '',
  ...
  const handleDeleteItem = () => {
    // delete item from SupplierInventory
    fetch(`${import.meta.env.VITE_BASE_URL}/supplier/deteleSupplier/${newItem.id}`, {
      method: 'DELETE',
    })
      .then((response) => {
        if (response.ok) {
          const updatedSupplierInventory = supplierInventoryData.filter((item) => item.id !== newItem.id);
          setSupplierInventoryData(updatedSupplierInventory);
          setNewItem({
            name: '',
            id: '',
  ...
  return (
    <div>
      ...
      <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
        <Header title="Supplier Inventory Items" />
